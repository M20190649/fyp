...
I would first like to thank my supervisor Dr. Charlie and Co-supervisor Dr.Colin Layfield who taught me amongst many other things one important approach and they kept persisting on it - \textit{To let the data speak for itself}. Discussing with them gradual achievements and caveats made it possible to foster insight.
...
\chapter{Conclusion}
 ...
 %TODO - https://news.transport.gov.mt/traffic-counts-in-santa-venera-2/

 
 %TODO - traffic assignment 
 
 %trip count nso vs trip count od matrix -> survey resolution is far less than one offered in OD matrix
 
%TODO - scaling of results - IMPORTNT - can be taken from linear regression model - If a road section model correlation is proved to be high and result are statistically significant

%TODO - sample bias in Transport malta trip count - survey is taking during the afternoon when people who travel at night might not be there - during the night trip count is very low - it is state in NHTS that most respondents generally round off to the nearest 5 minutes

%TODO - NHTS car trip count was taken on one particular day - ours was taken on the whole month - plot is smoother.

%TODO - google cloud api cannot give historical records on delays. Prediciton of delay for very far away is not based on historical records.

%TODO - controexamination between google distance matrix api and OD matrix trip delay results.
