...
\subsection{Cleaning of data, removal of noise and minimization of displacement error}
...
\subsection{Use of scaling methods to get more accurate OD matrices}
...
In equation \ref{OD_scaling}, $OD_{ij}$ is the final resulting actual OD matrix.  $(t-OD_{ij})$ is the transient OD matrix that contains trip data from origin nodes to destination nodes. Transient here means that the node to node trips may be missing the actual nodes' information because CDR data does not capture all locations in the trips made. Thus $(t-OD_{ij})$ represents only a segment of the actual trips. A simulation platform, MITSIMLab\footnote{https://its.mit.edu/software/mitsimlab}, is used to find a scaling factor $\beta_{ij}$ for every transient OD link.
